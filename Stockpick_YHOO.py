...
if __name__ == '__main__':
  ...
  while(1):
    ...
    for bar in xrange(0,len(listSymbol)):
      #print bar
      try:
        symbol=listSymbol[bar]
      #print symbol
        q = WeeklyQuote(symbol)
        p =  DailyQuote(symbol)
        print 'check'+symbol.replace('\n','')
     
        if(len(q.close)>200):
          if(q.avg50[len(q.close)-1]>q.avg50[len(q.close)-10]):
            trenddirection=1
          else:
            trenddirection=2
          if(p.close[len(p.close)-1]<(p.avg20[len(p.close)-1]-0.875*(p.avg20[len(p.close)-1]-p.bollinger_low[len(p.close)-1])) and trenddirection==1 and p.avg50[len(p.close)-1]<(p.avg20[len(p.close)-1]-0.5*(p.avg20[len(p.close)-1]-p.bollinger_low[len(p.close)-1]))):
            listOuput.append([symbol.replace('\n',''),'LONG'])
          if(p.close[len(p.close)-1]<(q.avg20[len(q.close)-1]-0.875*(q.avg20[len(q.close)-1]-q.bollinger_low[len(q.close)-1])) and trenddirection==1 and p.avg50[len(p.close)-1]<(p.avg20[len(p.close)-1]+0.5*(p.avg20[len(p.close)-1]-p.bollinger_low[len(p.close)-1]))):
            listOuput.append([symbol.replace('\n',''),'LONG+'])
          if(p.close[len(p.close)-1]>(p.avg20[len(p.close)-1]+0.875*(p.avg20[len(p.close)-1]-p.bollinger_low[len(p.close)-1])) and trenddirection==2 and p.avg50[len(p.close)-1]>(p.avg20[len(p.close)-1]+0.5*(p.avg20[len(p.close)-1]-p.bollinger_low[len(p.close)-1]))):
            listOuput.append([symbol.replace('\n',''),'SHORT'])
          if(p.close[len(p.close)-1]>(p.avg20[len(p.close)-1]+0.875*(p.avg20[len(p.close)-1]-p.bollinger_low[len(p.close)-1])) and trenddirection==2 and p.avg50[len(p.close)-1]>(p.avg20[len(p.close)-1]-0.5*(p.avg20[len(p.close)-1]-p.bollinger_low[len(p.close)-1]))):
            listOuput.append([symbol.replace('\n',''),'SHORT+'])
#
...
#  listOuput.append(['XOM','LONG'])
    ...
    if(len(listOuput)>1):
      ...
      send_email('wshenstock@gmail.com','bdbd0213','danbao84@gmail.com','StocktoCheck',listOuput)
    time.sleep(86400)
